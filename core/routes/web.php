...
// Agrega la ruta de prueba aquí, al final del archivo
Route::get('/test-db', function () {
    try {
        // Intenta obtener una conexión a la base de datos
        DB::connection()->getPdo();

        // Verifica si la tabla general_settings existe
        $result = DB::select('SHOW TABLES LIKE "general_settings"');

        if (empty($result)) {
            return 'La tabla general_settings NO existe en la base de datos: ' . DB::getDatabaseName();
        } else {
            // Intenta obtener el primer registro de la tabla
            $data = DB::table('general_settings')->first();
            if ($data) {
                 return 'Conexión a la base de datos exitosa. La tabla general_settings existe y se pudo leer el primer registro.';
            } else {
                 return 'Conexión a la base de datos exitosa. La tabla general_settings existe, pero no tiene registros.';
            }
